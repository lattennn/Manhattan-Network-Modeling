...
class l3_rx : public cSimpleModule
{
  private:
    int numRxed, ind, kind, ack_seq;
    ...
    double Module21_Avg; // Final delivery delay we're interested in finding.
    l3Msg *ACK_rxed; // The message sent to TX to confirm the arrival of acks.
    l3Msg *start; // Selfmessage to start simulation.
    l3Msg *ack; // Message to be sent as ack.
  ...
  protected:
    ...
    virtual void generateACK(l3Msg *msg);
...
void l3_rx::initialize()
{
    ...
    start = new l3Msg("start"); // Creating new self message.
    scheduleAt(simTime(), start); // Scheduling to current simulation time.
}

void l3_rx::handleMessage(cMessage *msg)
{
    // Kind 1 = Data 0
    // Kind 2 = Data 1
    // Kind 3 = Ack 0
    ...
    FSM_Switch(rx_fsm) {
        ...
        case FSM_Exit(WAIT_FOR_DATA_0):
            if (ttmsg-> getKind() == 1){ // If we get Data 0
                ...
                if(numRxed != 19000){ // If we have not received 19000 messages
                    if (ind == 21){ // If we're module G+2,
                        cMessage *copy = (cMessage *)msg->dup(); // We create a copy
                        l3Msg *ttmsg = check_and_cast<l3Msg *>(copy); // We cast the copy to our data type.
                        ...
                        delete ttmsg; // We dispose of the copy.
                    }
                    FSM_Goto(rx_fsm,FROM_0_TO_1); // finally, we move to the transition state between 0 and 1.
                }
                else{ // If we have received 19000 messages,
                    delete ack; // We delete the ack object,
                    FSM_Goto(rx_fsm,FINISH); // And proceed to the finish state.
                }
            }
        ...
        case FSM_Exit(SEND_TX_AT_0):
            ...
            ACK_rxed = new l3Msg("ack_rxed",kind); // Create ack_rxed for tx.
        ...
        case FSM_Exit(FROM_1_TO_0):
            ...
            generateACK(ttmsg); // We generate and send an ack to the sender.
            delete ttmsg; // We delete the data message.
            FSM_Goto(rx_fsm,WAIT_FOR_DATA_0); // Proceed to wait for data 0 message.
            break;

        case FSM_Exit(SEND_BACK_ACK_IN_1):
            generateACK(ttmsg); // We generate ack for data 1 message.
            delete ttmsg; // We delete data message.
            FSM_Goto(rx_fsm,WAIT_FOR_DATA_1); // Go back to waiting for 1.
            break;
    }
}

void l3_rx::generateACK(l3Msg *msg)
{
    ...
    l3Msg *ack = new l3Msg(ackname,kind); // Create ack message
    // Add fields to it.
    ack->setSEQ(ack_seq);
    ack->setSource(ind);
...
void l3_rx::finish()
{
    ...
    Module21_Avg = (double) delay_tot/numRxed; // Calculate delivery delay
    recordScalar("Mod21Avg", Module21_Avg); // Save data
    EV << "Total delay is " << delay_tot << " and I received " << numRxed << endl;
    }
}
