...
#include <string.h>
#include <omnetpp.h>
#include "l1t1cMsg_m.h"

using namespace omnetpp;


class l1t1c : public cSimpleModule
{
  private:
  ...
  protected:
    virtual l1t1cMsg *generateMessage();
    //Function for message generation.
    virtual void forwardMessage(l1t1cMsg *msg);
...
Define_Module(l1t1c); //Module registration

void l1t1c::initialize()
{
    ...
    if (getIndex() == 0) {
        ...
        // collected.
        l1t1cMsg *msg = generateMessage();
        scheduleAt(0.0, msg); //Increase the number of messages sent by 0.
    }
}

void l1t1c::handleMessage(cMessage *msg)
{
    l1t1cMsg *ttmsg = check_and_cast<l1t1cMsg *>(msg);
    // Check that the received message belongs to class l1t1bMsg, our predefined message class.
    ...
    if (ttmsg->getDestination() == getIndex()) {
        ...
        l1t1cMsg *newmsg = generateMessage();// Generation.
        EV << newmsg << endl;
...
l1t1cMsg *l1t1c::generateMessage()
{
    ...
    l1t1cMsg *msg = new l1t1cMsg(msgname);
    msg->setSource(src);
    msg->setDestination(dest);
    return msg; // Return object to calling module.
}

void l1t1c::forwardMessage(l1t1cMsg *msg)
{
